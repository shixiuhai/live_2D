...
class XunFeiVoiceBot:
    def __init__(self,appid,api_secret,api_key) -> None:
        self.appid = appid
    ...
    def generate_voice(self, text:str):
        # 测试时候在此处正确填写相关信息即可运行
        self.wsParam = Ws_Param(APPID=self.appid, APISecret=self.api_secret,
                        APIKey=self.api_key,
                        Text=text)
...
if __name__ == "__main__":
    xunfei=XunFeiVoiceBot(
        config.XUNFEI_VOICE_APPID,
        config.XUNFEI_VOICE_APISECRET,
        config.XUNFEI_VOICE_APIKEY
    )
    xunfei.generate_voice("你好呀，欢迎使用语音生产呀")
